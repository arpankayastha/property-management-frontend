...
(function ($) {
    ...
    var winnerMatchSources = function (teams, m) {
        return function () {
            var teamA = new TeamBlock(endOfBranch, function () {
                return teams[m][0];
            }, Option.of(Order.first()), Option.of(m * 2), Score.empty());
            var teamB = new TeamBlock(endOfBranch, function () {
                return teams[m][1];
            }, Option.of(Order.second()), Option.of(m * 2 + 1), Score.empty());
        ...
        if (isSingleElimination) {
            ...
            if (teams.length > 1 && !opts.skipConsolationRound) {
                ...
                var consol_1 = round.addMatch(function () {
                    return [{source: third_1}, {source: fourth_1}];
    ...
    function teamElement(roundNumber, match, team, opponent, isReady, isFirstBracket, opts, resultId, topCon, renderAll) {
        ...
        if (team.name.val !== undefined && team.name.val !== null && team.name.val.id !== undefined) {
            tEl.attr("data-participantid", team.name.val.id)
            // tEl.attr("data-score", team.name.val.score)
            tEl.attr("data-score", team.score.val)
        }
    ...
    var Match = /** @class */ (function () {
        function Match(round, match, seed, results, renderCb, isFirstBracket, opts, resultId, topCon, renderAll) {
            ...
            match.a.score = results.map(function (r) {
                return r.first.toNull();
            });
            match.b.score = results.map(function (r) {
                return r.second.toNull();
            });
